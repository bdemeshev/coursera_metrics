...
autoplot(y)  # график ряда
Acf(y)
Pacf(y)
ggtsdisplay(y)  # все три графика одним махом
...
getSymbols(Symbols = "GOOG", from = "2014-01-01", to = "2016-05-11", src = "yahoo")
...
ggtsdisplay(y)  # три графика для исходного ряда
dy <- diff(y)
ggtsdisplay(dy)  # три графика для приращений исходного ряда (для первой разности)
...
# упрощенная версия максимального правдоподобия не прошла запускаем полную, 
# это чуть больше времени занимает
