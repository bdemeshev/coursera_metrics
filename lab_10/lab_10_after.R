...
model_rf <- ranger(data = f_train, price ~ totsp + kitsp + livesp + brick) # в видеолекциях был более медленный RandomForest
...
yhat_rf <- predict(model_rf, f_test)$predictions # другой синтаксис для пакета ranger

# сумма квадратов остатков прогнозов по тестовой выборке: МНК
sum((y - yhat_lm) ^ 2)
# сумма квадратов остатков прогнозов по тестовой выборке: случайный лес
sum((y - yhat_rf) ^ 2)
